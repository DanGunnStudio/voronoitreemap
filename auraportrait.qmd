...
```{r}
# Add labels to the blurred image
for (i in seq_len(nrow(label_positions))) {
 aura_motion_blur <- aura_motion_blur |> 
    image_annotate(
      text = label_positions$label[i],
      size = 20,  # Adjust font size as needed
      location = paste0("+", round(label_positions$x[i]), "+", round(label_positions$y[i])),
...
```{r}
# Load the blurred image (assuming it's already loaded as aura_motion_blur)
labeled_image <- aura_motion_blur 

# Add labels to the image using a loop
for (i in seq_len(nrow(label_positions))) {
  # Create a temporary annotation layer for each label
  label_layer <- image_blank(
    width = image_info(labeled_image)$width,
    height = image_info(labeled_image)$height,
    color = "none"
  ) |> 
    image_annotate(
      text = label_positions$label[i],
  ...
  # Composite the label layer onto the base image
  labeled_image <- image_composite(labeled_image, label_layer)
}
```
...
for (i in seq_len(nrow(label_positions))) {
  text(
    x = round(label_positions$x[i]),
    y = round(label_positions$y[i]),
    labels = label_positions$label[i],
...
# Create a blank annotation layer with the same dimensions as the base image
annotation_layer <- image_blank(
  width = image_info(blurred_image)$width,
  height = image_info(blurred_image)$height,
  color = "none"  # Transparent background
)

# Loop through label_positions to annotate each label
for (i in seq_len(nrow(label_positions))) {
  # Add each label to the annotation layer
...
# Composite the annotation layer onto the blurred base image
final_image <- image_composite(blurred_image, annotation_layer)

# Save and display the final image
image_write(final_image, "final_treemap_with_labels.png")
print(final_image)
```
